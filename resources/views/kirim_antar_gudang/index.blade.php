  ...
  <div class="col-xs-12">
    <div class="box">
      ...
      <div class="box-header">
        ...
        <a href="{{ route('kirim_antar_gudang_detail.index') }}" class="btn btn-info"><i class="fa fa-plus-circle"></i> Surat Jalan Aktif</a>
        @endif
        @if($data !== 0)
...
@include('kirim_antar_gudang.detail')
@include('kirim_antar_gudang.unit')
@include('kirim_antar_gudang.list')

@if(session()->has('cetak'))
<script type="text/javascript">
  tampilPDF();
  function tampilPDF(){
    window.open("{{ route('kirim_antar_gudang.cetak',Session::get('cetak')) }}");
  }              
</script>
@endif
...
$(function(){
   table = $('.tabel-pembelian').DataTable({
     ...
     "serverside" : true,
     "ajax" : {
       "url" : "{{ route('kirim_antar_gudang.data') }}",
       "type" : "GET"
     }
   }); 
...
function showDetail(id){
    ...
    table1.ajax.url("kirim_antar_gudang/"+id+"/lihat");
    table1.ajax.reload();
}
