...
@include('stock_opname_toko.produk')
...
function addItem(){
   $.ajax({
       url:"{{ route('stock_opname_toko.store')}}",
       type: "POST",
    ...
    if(confirm("Data Akan Di Simpan?")){
        ...
        if(jumlah_detail != jumlah_stok){
                if(confirm("Data tidak sama, Lanjutkan?")){
                    $.ajax({
                        url: "stock_opname_toko/simpan_/"+kode,
                        type: "GET",
                        data: kode,
                        success: function(data){
